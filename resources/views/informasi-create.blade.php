...
@section('body')
    ...
    <div class="p-6 bg-white rounded-md shadow-md">
        <h2 class="text-lg text-gray-700 font-semibold capitalize">Add Informasi</h2>
        <hr class="my-3">
        <form action="{{ route('informasi.create') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex">
                    <div class="md:w-1/2 w-full pr-1">
                        ...
                        <select name="kategori"
                            class="form-input w-full px-4 py-2 appearance-none rounded-md focus:border-green-600 border border-gray-400 capitalize">
            ...
            <div class="mt-2">
                ...
                <textarea name="deskripsi" placeholder="Masukan deskripsi informasi"
                    class="form-input w-full px-4 py-2 appearance-none rounded-md focus:border-green-600 border border-gray-400">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <small class="text-sm text-red-500">{{ $message }}</small>
                @enderror
            </div>
            <div class="mt-2">
                ...
                <div class="focus:border-green-600 border border-gray-400 rounded-md overflow-hidden">
                    <textarea name="isi" id="wysiwygForm" placeholder="Masukan isi informasi">{{ old('isi') }}</textarea>
                </div>
                @error('isi')
                    <small class="text-sm text-red-500">{{ $message }}</small>
                @enderror
