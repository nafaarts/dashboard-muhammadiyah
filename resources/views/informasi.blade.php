...
@section('body')
    <div class="flex flex-col">
        ...
        <div class="flex justify-between mb-5">
            <a href="{{ route('informasi.create') }}" class="bg-green-700 text-white rounded-md text-sm py-1 px-4"><i
                    class="fas fa-fw fa-add"></i> Tambah
                Informasi</a>
            <a href="{{ route('kategori-informasi') }}" class="bg-gray-600 text-white rounded-md text-sm py-1 px-4"><i
                    ...
                    <tbody class="bg-white">
                        @foreach ($data as $item)
                            <tr>
                                    ...
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            <img class="h-10 w-10 rounded-full"
                                                src="{{ asset('img/informasi/thumbnail/' . $item->gambar) }}" alt="" />
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                    <span
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100">{{ $item->views }}</span>
                                </td>
                                ...
                                <td
                                    ...
                                    <a href="{{ route('informasi.edit', $item) }}"
                                        class="text-gray-600 hover:text-gray-900"><i class="fas fa-fw fa-edit"></i></a>
                                    <form action="{{ route('informasi.delete', $item) }}" method="POST"
