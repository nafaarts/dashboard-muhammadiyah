...
@section('body')
    ...
    @endif
    <form action="{{ route('kategori-informasi.store') }}" method="POST">
        @csrf
        <div class="flex">
            <div class="w-5/6">
    ...
    <div class="bg-yellow-100 rounded-lg p-4 mt-4 text-sm text-yellow-700 shadow" role="alert">
        <div>
            ...
            <span class="font-medium">Peringatan!</span> Kategori tidak bisa dihapus jika terdapat informasi.
        </div>
    </div>
        ...
        <table class="text-left w-full border-collapse">
            <thead class="border-b">
                <tr>
                    ...
                    <th class="py-3 px-5 bg-gray-50 font-medium uppercase text-sm text-gray-500">Jumlah Informasi</th>
            ...
            <tbody>
                @foreach ($data as $item)
                    <tr class="hover:bg-gray-50 kategori-row">
                        ...
                        <td class="py-4 px-6 border-b text-gray-500 text-sm">
                            <strong>{{ $item->informasis->count() }}</strong>
                            Informasi terdaftar
                        </td>
                        ...
                        <td class="py-4 px-6 border-b text-gray-500">
                            ...
                            @if (!$item->informasis->count())
                                <form action="{{ route('kategori-informasi.delete', $item->id) }}" method="POST"
        ...
        row.forEach(element => {
            element.childNodes[7].childNodes[1].addEventListener('click', (e) => {
                ...
                let el2 = `<td colspan="4" class="py-4 px-6 border-b text-gray-700 bg-gray-50">
                    <form action="{{ route('kategori-informasi.update') }}" method="POST">
                        @csrf
                        @method('PATCH')
